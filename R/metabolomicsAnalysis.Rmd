...
To visualize the results, we can use a Tukey pos-hoc test OR estimated marginal means (least squares means) from the `emmeans` package.
...
for(i in 1:length(Tmetabs)) {
  ...
  # perform least-squares means
  e <- as.data.frame(emmeans(mod, pairwise ~ Time)$contrasts)
  e$metabolite <- Tmetabs[i]

  # append to outDF
  out <- rbind(out, e)
}

sigsdf <- out[out$p.value < 0.05 , ]
knitr::kable(sigsdf)
```
...
plotdf <- plasma[plasma$metabolite %in% sigsdf$metabolite, ]

ggplot(data = plotdf, aes(x = Time, y = area)) +
  geom_hline(yintercept = 0, color = "red") +
  geom_boxplot() +
  facet_wrap(~metabolite) 
```
...
for(i in 1:length(Tmetabs)) {
  ...
  # perform least-squares means
  e <- as.data.frame(emmeans(mod, pairwise ~ treatmentID)$contrasts)
  e$metabolite <- Tmetabs[i]
...
In this test there are several significant metabolites, however - a Tukey post-hoc test shows no significant differences.
...
```{r}
## too many to look at boxplots - instead look at which group is higher
...
However, there are too many significant metabolites to utilize boxplots, so the table below shows means and standard deviations for each metabolite.
