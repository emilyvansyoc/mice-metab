...
tomatch <- aq %>% 
  ...
  mutate(newmetabolite = 
           str_remove(metabolite , "X")) %>% 
  mutate(newmetabolite  = str_replace_all(newmetabolite , "\\.", "-")) %>% 
  mutate(newmetabolite  = str_remove_all(newmetabolite , "-$")) %>% 
  mutate(newmetabolite  = as.character(newmetabolite ))

# merge
both <- tomatch %>% 
  left_join(keggs, by = c("newmetabolite" = "Query"))

# get just vector of metabolite names for plots
names <- both %>% 
  select(metabolite, newmetabolite, Match) %>% 
  distinct() %>% 
  rename(oldnames = metabolite,
         cleanednames = newmetabolite,
         KEGGMatch = Match)
...
# write.table(assignv, file = "./data/aqeuousKEGG_Assignments.txt", sep = "\t", row.names = FALSE)
